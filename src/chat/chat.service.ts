...
export class ChatService {
  ...
  async sendMessage(userEmail: string, message: string, conversationId?: string) {
    ...
    try {
        const routerResponse = await this.promptRouterService.routePrompt(message);
        aiResponseText = routerResponse;
        this.logger.log(`Router processed for ${userEmail}: ${aiResponseText.substring(0, 50)}...`);
